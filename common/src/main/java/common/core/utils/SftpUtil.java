...
public class SftpUtil {
    ...
    public boolean uploadBatchFiles(String remotePath, String localPath, boolean isDel)
        ...
        for (File file : files) {
            if (file.isFile()) {
                if (!".".equals(file.getName()) && !"..".equals(file.getName())) {
                    boolean bool =
                            this.uploadFile(localPath, file.getName(), remotePath, file.getName());
                    if (bool && isDel) {
        ...
        log.info(
                "Sftp批量文件上传--->本地目录[{}], 文件数量[{}], 远程目录[{}]--->上传成功！",
                localPath,
                files.length,
                remotePath);
        return true;
    ...
    public void downBatchFiles(String remotePath, String localPath, boolean isMonitor)
        ...
        log.info(
                "Sftp批量下载文件--->远程目录[{}], 本地目录[{}], 文件数量[{}]--->处理成功！",
                remotePath,
                localPath,
                list.size());
    }
    ...
    private void getSftpLsEntry4Dir(
            SftpLsEntry sftpLsEntry,
            LinkedList<SftpLsEntry> dirList,
            LinkedList<SftpLsEntry> pathList)
            throws Exception {
        ...
        sftpFiles.forEach(
                lsEntry -> {
                    ...
                    if (!isSpecialSFileName(fileName)) {
                        ...
                        SftpLsEntry newLsEntry =
                                new SftpLsEntry(fileName, path, true, attr.getSize());
                        if (attr.isDir()) {
